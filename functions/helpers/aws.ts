...
export async function uploadConversationToS3(context: Context, callSid: string, threadId: string) {
  ...
  await uploadObjectToS3(s3Client, context.AWS_S3_BUCKET, `${callSid}.json`, JSON.stringify(conversation))
}
...
export async function uploadAudioToS3(context: Context, audioFileName: string, buffer: Buffer) {
  ...
  await uploadObjectToS3(s3Client, "eleven-labs-bucket", audioFileName, buffer)
}

async function uploadObjectToS3(s3Client: S3Client, bucketName: string, key: string, data: StreamingBlobPayloadInputTypes) {
  await s3Client.send(
